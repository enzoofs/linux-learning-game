...
export function Journal() {
  ...
  return (
    <div className="p-6 max-h-[70vh] overflow-y-auto">
      <div className="text-sm text-slate-500 mb-1 font-semibold tracking-wider">COMMAND JOURNAL</div>
      <div className="text-xs text-slate-600 mb-6">
        {completedDrills.length} commands practiced
      </div>

      {completedModuleData.length === 0 ? (
        <div className="text-slate-600 text-sm text-center py-12">
          Complete modules to fill your journal!
        </div>
      ) : (
          ...
          return (
            <div key={mod.id} className="mb-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-slate-500">{mod.tier}</span>
              </div>
